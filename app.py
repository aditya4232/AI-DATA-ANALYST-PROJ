...
SAMPLE_QUESTIONS = [
    ...
    "Show the top 2 products by total sales.",
...
def main() -> None:
    ...
    with header_col:
        st.markdown(
            ...
            <div class="hero">
                ...
                <p>Upload a CSV, ask analytical questions in plain English, and get reproducible pandas results with charts.</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with st.sidebar:
        st.header("Workspace")
        st.caption("Configure the model provider and upload your dataset.")
        provider = st.text_input("Provider", value=config.provider)
        model = st.text_input("Model", value=config.model)
        base_url = st.text_input("Base URL", value=config.base_url)
        api_key = st.text_input(
            "API key override",
            value=os.getenv("NVIDIA_API_KEY", os.getenv("OPENAI_API_KEY", config.api_key)),
            type="password",
        )
        st.divider()
    ...
    if not uploaded_file:
        st.info("Upload a CSV to begin. The app will profile the data before answering any questions.")
        return
    ...
    profile = build_profile(df)
    resolved_config = config.resolved(api_key=api_key, model=model, base_url=base_url)
    resolved_config = AppConfig(
        ...
        max_rows_preview=config.max_rows_preview,
    )
        ...
        if not st.session_state["history"]:
            ...
            for index, item in enumerate(reversed(st.session_state["history"]), start=1):
                with st.expander(f"{index}. {item['question']}", expanded=index == 1):
                    ...
                    st.caption(f"Result type: {item.get('result_type', 'None')}")
